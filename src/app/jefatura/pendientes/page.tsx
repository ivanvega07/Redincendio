...
export default async function JefaturaPendientesPage() {
  ...
  const planillas = await prisma.planilla.findMany({
    ...
    include: { seccion: true, jefe: { select: { nombre: true, apellido: true } } },
    orderBy: [{ anio: "desc" }, { mes: "desc" }],
  });

  return (
    <AppLayout title="Pendientes de Revisión">
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <ClipboardCheck className="h-5 w-5 text-bombero-rojo" />
          <div>
            <h2 className="font-semibold text-gray-800">Planillas esperando revisión</h2>
        ...
        {planillas.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-gray-500">No hay planillas pendientes de revisión.</p>
            </CardContent>
          </Card>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium">Firmado por</th>
              ...
              <tbody className="divide-y">
                {planillas.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{formatFecha(p.fechaFirmaJefe ?? p.updatedAt)}</td>
